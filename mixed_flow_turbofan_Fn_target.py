...
class MixedFlowTurbofan(pyc.Cycle):
    ...
    def setup(self):
        ...
        if design:
            ...
            balance.add_balance('BPR', eq_units=None, lower=0.25, upper=0.8, val=0.5) #OG val=5.0
            ...
            balance.add_balance('BPR', lower=0.25, upper=1.0, eq_units='psi') #OG upper=5.0
            self.connect('balance.BPR', 'splitter.BPR')
...
class MPMixedFlowTurbofan(pyc.MPCycle):
    def setup(self):
        ...
        self.set_input_defaults('DESIGN.LP_Nmech', 10000, units='rpm') #4600
        self.set_input_defaults('DESIGN.HP_Nmech', 16800, units='rpm') #14000
        ...
        self.od_Fns = [17700] #target SL thrust
        self.od_alts = [0.1]
        self.od_MNs = [0.01]
        ...
        for i,pt in enumerate(self.od_pts):
            ...
            #self.set_input_defaults(pt+'.balance.rhs:FAR_ab', self.od_T7s[i], units='degR') #balances for AB temp
...
if __name__ == "__main__":
    ...
    prob.set_val('DESIGN.fc.MN', 0.85)
    
    prob.set_val('DESIGN.balance.rhs:W', 4021.0, units='lbf') #Target Thrust
    prob.set_val('DESIGN.balance.rhs:FAR_core', 3200, units='degR') # Target Tt4
    prob.set_val('DESIGN.balance.rhs:FAR_ab', 3600, units='degR')
    
    prob.set_val('DESIGN.fan.PR', 4)
    ...
    prob['DESIGN.balance.BPR'] = 0.35
    prob['DESIGN.balance.W'] = 90.
    prob['DESIGN.balance.lpt_PR'] = 3.0 #3.5
    prob['DESIGN.balance.hpt_PR'] = 3.0 #2.5
    prob['DESIGN.fc.balance.Pt'] = 5.2
    prob['DESIGN.fc.balance.Tt'] = 444
    prob['DESIGN.mixer.balance.P_tot']= 16 #OG: 15. Having this over 10^1 off caused mixer backflow, I think, which screwed up everything downstream

    for i,pt in enumerate(mp_mixedflow.od_pts):

        prob[pt+'.balance.FAR_core'] = 0.025
        prob[pt+'.balance.FAR_ab'] = 0.025
        prob[pt+'.balance.BPR'] = 0.4 #2.5
        prob[pt+'.balance.W'] = 146. #50
        prob[pt+'.balance.HP_Nmech'] = 14000
        prob[pt+'.balance.LP_Nmech'] = 4000
        prob[pt+'.fc.balance.Pt'] = 5.2
        prob[pt+'.fc.balance.Tt'] = 444
        prob[pt+'.mixer.balance.P_tot']= 16 #OG: 15. Having this over 10^1 off caused mixer backflow, I think, which screwed up everything downstream
        prob[pt+'.hpt.PR'] = 3.0
        prob[pt+'.lpt.PR'] = 3.0
